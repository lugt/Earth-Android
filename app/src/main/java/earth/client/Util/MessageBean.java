...
public class MessageBean {
    ...
    public String toString(){
        ...
        byte[] ms = Base64.encode(msg.getBytes(), Base64.NO_WRAP);
        len = ms.length;
        ...
        byte[] byte_et = Base64.encode(byte_init,Base64.NO_WRAP);
        byte[] out = new byte[len + byte_et.length];

        System.arraycopy(byte_et,0,out,0,byte_et.length);
        System.arraycopy(ms,0,out,byte_et.length,len);
    ...
    public CharSequence getInlineMsg() {
        if (msg.length() > 10) {
