...
public class ClientHandler extends ChannelInboundHandlerAdapter {
    ...
    private static final String TAG = "Short:";
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        if (msg instanceof HttpResponse) {
            ...
            if(response.status().equals(HttpResponseStatus.BAD_GATEWAY)){
                ...
                Monitor.logger(TAG,"Client : Request 404" + Arrays.toString(((HttpResponse) msg).headers().entries().toArray()));
            }else if(response.status().equals(HttpResponseStatus.BAD_REQUEST)){
                //402
                // bad request
                Monitor.logger(TAG,"Client : Request 400" + Arrays.toString(((HttpResponse) msg).headers().entries().toArray()));
            }else if(response.status().equals(HttpResponseStatus.FORBIDDEN)){
                //403
                // forbidden
                Monitor.logger(TAG,"Client : Request 403" + Arrays.toString(((HttpResponse) msg).headers().entries().toArray()));
            }else if(response.status().equals(HttpResponseStatus.OK)){
                // OK
            }else if(response.status().equals(HttpResponseStatus.GATEWAY_TIMEOUT)){
                //
                Monitor.logger(TAG,"Client : Request Timeout" + Arrays.toString(((HttpResponse) msg).headers().entries().toArray()));
            }else{
                Monitor.logger(TAG,"Client : Request Has Failed" + Arrays.toString(((HttpResponse) msg).headers().entries().toArray()));
            }
