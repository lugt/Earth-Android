...
public class LocalHandler implements ShortHandlerInterface {
    ...
    private static void checkServerAvailability(String[] m) {
        try {
            int version = Integer.valueOf(m[1]);
            if (Constant.ClientV >= version) {
                // 最新版本，无需更新
                hdl.obtainMessage(1000, "update").sendToTarget();
            } else {
                // 判断相关版本更新
    ...
    private void unUsual(String[] m) {
        Monitor.logger("Unusual Signal Rcvd : " + m[1]);
    }
    ...
    private void loginReturn(String[] m) {
        if("ok".equals(m[1])) {
            ...
            Monitor.logger("Error : PassAuthAct lg failed - " + m[2] + "," + (m.length >= 4?m[3]:""));
        }
    ...
    private void unHandled(String[] m) {
        Monitor.logger("Client Rcvd : " + m[0]+","+m[1]);
    ...
    public void longStatus(int status, int result, Object obj) {
        ...
        if(status == -20006) {
            ...
            hdl.obtainMessage(-517).sendToTarget();
        }else{
    ...
    private void except(XLResponse rep) {
        // get
        String t = rep.getValues().values().toString();
        Monitor.logger("收到了长链接返回的错误信息:" + rep + " ; values:"+t);
    ...
    private int receiveOneMsg(String s) {
        try {
            ...
            // Call UI Thread...
            if(hdl instanceof MainActivityHandler) {
            ...
            /// Handling
            Monitor.error(msB.getFromId() + " said : " + msB.msg);
            // 回复1000为成功，其他为失败
            return 1000;
        } catch (Exception e) {
            e.printStackTrace();
